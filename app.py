...
try:
    # Optional: pywebpush handles VAPID signing + payload encryption for Web
    # Push (RFC 8291/8292). Not stdlib — add "pywebpush" to requirements.txt
    # to enable real push notifications. Without it, push calls are quietly
    # skipped so the rest of the app keeps working.
    from pywebpush import webpush, WebPushException
except ImportError:
    webpush = None
    class WebPushException(Exception):
        response = None
    print("[push] pywebpush not installed — push notifications are disabled. "
          "Add 'pywebpush' to requirements.txt and set VAPID_PUBLIC_KEY / "
          "VAPID_PRIVATE_KEY to enable them.")
...
def send_push(user_id, category, title, body, url="/chat", tag=None):
    ...
    honoring their per-category preference. Silently no-ops if pywebpush
    isn't installed or VAPID keys aren't configured — the in-app/socket
    notification still works either way, this is only the "app is closed"
    channel."""
    if webpush is None:
        return
    public_key, private_key = os.environ.get("VAPID_PUBLIC_KEY"), os.environ.get("VAPID_PRIVATE_KEY")
    ...
    if not notification_prefs(user).get(category, True):
        conn.close()
        return
    subs = conn.execute("SELECT id,endpoint,p256dh,auth FROM push_subscriptions WHERE user_id=?",
                        (user_id,)).fetchall()
    conn.close()
    if not subs:
        return
    payload = json.dumps({"title": title, "body": body, "url": url, "tag": tag or category})
    claims = {"sub": "mailto:" + os.environ.get("VAPID_CONTACT_EMAIL", "admin@example.com")}
    stale_ids = []
    for sub in subs:
        try:
            webpush(
            ...
            if status in (404, 410):  # subscription expired/unsubscribed on the browser's end
                stale_ids.append(sub["id"])
            else:
                print(f"[push] send failed for user {user_id}: {exc}")
        except Exception as exc:  # noqa: BLE001 - keep the request path alive
            print(f"[push] unexpected error for user {user_id}: {type(exc).__name__}: {exc}")
    if stale_ids:
        conn = db()
...
def vapid_public_key():
    """Public — like the Google client ID above, the VAPID *public* key is
    meant to be embedded in the browser (it's what pushManager.subscribe()
    needs). The private key stays server-side in VAPID_PRIVATE_KEY."""
...
def push_subscribe(user_id):
    ...
    conn = db()
    conn.execute("""INSERT INTO push_subscriptions(user_id,endpoint,p256dh,auth,device_id)
                    VALUES(?,?,?,?,?)
                    ON CONFLICT(endpoint) DO UPDATE SET
                      user_id=excluded.user_id, p256dh=excluded.p256dh,
                      auth=excluded.auth, device_id=excluded.device_id""",
                (user_id, endpoint, p256dh, auth_key, data.get("device_id")))
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    ...
    if not (os.environ.get("TURN_URL") and os.environ.get("TURN_USERNAME")
            and os.environ.get("TURN_CREDENTIAL")):
        print("[calls] No TURN_URL/TURN_USERNAME/TURN_CREDENTIAL set — calls "
              "will only work when both users are on networks that allow "
              "direct/STUN connections. Behind carrier-grade NAT (common on "
              "Nigerian mobile data) both sides will hang at 'Connecting…' "
              "and time out. Add a TURN relay (e.g. Open Relay or Metered) "
              "via those three env vars to fix this.")
    if not (os.environ.get("VAPID_PUBLIC_KEY") and os.environ.get("VAPID_PRIVATE_KEY")):
        print("[push] VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY not set — push "
              "notifications will not be delivered when HwFar is closed, "
              "even after a user grants browser permission.")
