...
# A disconnect doesn't necessarily mean the person left: navigating between
# HwFar's server-rendered pages (chat -> settings -> chat), a phone locking
# its screen, or a few seconds of flaky network all close the socket even
# though the person is still on the network and hasn't turned off their
# online status. Rather than flipping them to "offline" (and stamping
# last_seen) the instant every socket drops, give each person a short grace
# window to reconnect before treating the disconnect as real. pending_offline
# holds one scheduled eventlet timer per user_id; a reconnect within the
# window cancels it and nothing is ever broadcast as offline.
pending_offline = {}
...
def init_db():
    ...
    schema_sql = """
      ...
      CREATE TABLE IF NOT EXISTS warnings (
        ...
        message TEXT NOT NULL, issued_by TEXT DEFAULT 'HwFar Admin',
        created_at TEXT DEFAULT CURRENT_TIMESTAMP, acknowledged INTEGER DEFAULT 0
      );
    ...
    for table, column, ddl in [
        ...
        ("contacts", "muted", "muted INTEGER DEFAULT 0"),
        ...
        ("users", "banned", "banned INTEGER DEFAULT 0"),
        ("users", "ban_reason", "ban_reason TEXT"),
        ("users", "banned_at", "banned_at TEXT"),
        ("users", "is_official", "is_official INTEGER DEFAULT 0"),
        ("reports", "status", "status TEXT DEFAULT 'open'"),
        ("reports", "resolved_at", "resolved_at TEXT"),
        ("reports", "resolution_note", "resolution_note TEXT"),
    ]:
        if DATABASE_URL:
    ...
    # Seed a single "official" account that the admin panel sends chat
    # announcements from. It has no usable password (random, never shown)
    # so nobody can log into it directly; is_official=1 marks it so it's
    # excluded from suggestions/reporting and can be recognized by the
    # frontend if you choose to badge it.
    existing_official = conn.execute(
        "SELECT id FROM users WHERE username=?", (OFFICIAL_USERNAME,)).fetchone()
    if not existing_official:
        conn.execute(
            """INSERT INTO users(username,password_hash,discoverable,show_online,is_official)
               VALUES(?,?,0,0,1)""",
            (OFFICIAL_USERNAME, generate_password_hash(secrets.token_urlsafe(32))))
    conn.commit()
    conn.close()


OFFICIAL_USERNAME = os.environ.get("OFFICIAL_ACCOUNT_USERNAME", "HwFar Official")
init_db()
...
# --- Admin panel -------------------------------------------------------
# Separate credential pair from regular user accounts, configurable via
# env vars so the defaults below aren't the only way in on a real
# deployment. These log into /admin with their own JWT (kind="admin"),
# never a regular user token, and that token is never accepted by @auth.
ADMIN_USERNAME = os.environ.get("ADMIN_USERNAME", "ConsistencyHwFar")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "HwfarApp")


def make_admin_token():
    return jwt.encode(
        {"kind": "admin", "sub": ADMIN_USERNAME,
         "exp": dt.datetime.utcnow() + dt.timedelta(hours=12)},
        app.config["SECRET_KEY"], algorithm="HS256")


def admin_from_token(value):
    try:
        claims = jwt.decode(value, app.config["SECRET_KEY"], algorithms=["HS256"])
        return claims.get("sub") if claims.get("kind") == "admin" else None
    except jwt.InvalidTokenError:
        return None


def admin_auth(handler):
    """Require a valid admin token — entirely separate from @auth, which
    only ever accepts regular user tokens. An admin token never grants
    access to user endpoints, and a user token never grants access to
    admin endpoints."""
    @wraps(handler)
    def wrapped(*args, **kwargs):
        header = request.headers.get("Authorization", "")
        admin_name = admin_from_token(header.split(" ", 1)[1]) if header.startswith("Bearer ") else None
        if not admin_name:
            return jsonify(error="admin login required"), 401
        return handler(admin_name, *args, **kwargs)
    return wrapped


def log_admin_action(actor, action, target="", detail=""):
    conn = db()
    conn.execute("INSERT INTO admin_actions(action,target,detail) VALUES(?,?,?)",
                (f"{action} (by {actor})", target, detail))
    conn.commit()
    conn.close()


def official_user_id(conn):
    row = conn.execute("SELECT id FROM users WHERE username=?", (OFFICIAL_USERNAME,)).fetchone()
...
def auth(handler):
    ...
    def wrapped(*args, **kwargs):
        ...
        if is_banned(user_id):
            return jsonify(error="your account has been suspended", banned=True), 403
...
def online(user_id):
    # A live socket obviously counts. So does being inside the short grace
    # window right after the last one dropped (see pending_offline) — that
    # covers someone navigating between HwFar pages, their screen locking
    # for a moment, or a brief network blip, so a /contacts or /presence
    # request during that gap still reports them online instead of flashing
    # "offline" ahead of the socket-side presence broadcast.
    return bool(online_users.get(user_id)) or user_id in pending_offline
...
def status_payload(conn, row, viewer_id):
    ...
    return {
        ...
        "view_count": conn.execute(
            "SELECT COUNT(*) AS count FROM status_views WHERE status_id=?", (row["id"],)
        ).fetchone()["count"],
        "viewers": viewers,
...
def login():
    ...
    if row["banned"]:
        reason = f" Reason: {row['ban_reason']}" if row["ban_reason"] else ""
        return jsonify(error=f"This account has been suspended.{reason}", banned=True), 403
...
def account_me(user_id):
    ...
    result["banned"] = bool(row["banned"]) if "banned" in row.keys() else False
    return jsonify(result)


@app.get("/account/warnings")
@auth
def account_warnings(user_id):
    """A user's own account warnings/strikes — never exposed for anyone
    but the account owner, since this route is scoped to the caller's
    own user_id from their token, not a lookup by name."""
    conn = db()
    rows = conn.execute("""SELECT id,message,issued_by,created_at,acknowledged
      FROM warnings WHERE user_id=? ORDER BY id DESC""", (user_id,)).fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])


@app.post("/account/warnings/<int:warning_id>/ack")
@auth
def acknowledge_warning(user_id, warning_id):
    conn = db()
    cur = conn.execute("UPDATE warnings SET acknowledged=1 WHERE id=? AND user_id=?",
                       (warning_id, user_id))
    conn.commit()
    conn.close()
    if not cur.rowcount:
...
def contacts(user_id):
    ...
    rows = conn.execute("""
      SELECT * FROM (
        SELECT u.id,u.username,u.avatar,u.last_seen,u.banned,u.is_official,
          ...
          c.archived AS archived, c.muted AS muted
    ...
    for row in rows:
        ...
        item["muted"] = bool(row["muted"])
        item["banned"] = bool(row["banned"])
        item["is_official"] = bool(row["is_official"])
...
def suggestions(user_id):
    conn = db()
    rows = conn.execute("""
      SELECT id,username,avatar,country,age,gender,show_age,show_gender,last_seen,banned,is_official
...
def profile(user_id, name):
    conn = db()
    row = conn.execute("""SELECT id,username,avatar,country,age,gender,show_age,show_gender,last_seen,banned,is_official
...
@app.post("/contacts/<name>/mute")
@auth
def mute_contact(user_id, name):
    data = request.get_json(silent=True) or {}
    muted = 1 if data.get("muted", True) else 0
    conn = db()
    target = conn.execute("SELECT id FROM users WHERE username=?", (name,)).fetchone()
    if not target:
        conn.close()
        return jsonify(error="no such user"), 404
    cur = conn.execute("UPDATE contacts SET muted=? WHERE owner_id=? AND contact_id=?",
                       (muted, user_id, target["id"]))
    conn.commit()
    conn.close()
    if not cur.rowcount:
        return jsonify(error="that user is not in your contacts"), 404
    return jsonify(muted=bool(muted), username=name)
...
def report_user(user_id, name):
    ...
    conn.commit()
    reporter = conn.execute("SELECT username FROM users WHERE id=?", (user_id,)).fetchone()
    conn.close()
    socketio.emit("admin_new_report", {
        "reporter": reporter["username"] if reporter else "?",
        "reported": name, "reason": reason,
    }, room="admin_live")
...
# ADMIN PANEL
# -------------------------------------------------------------------------
# Everything below is gated by @admin_auth (a separate credential/token
# from regular user accounts — see ADMIN_USERNAME/ADMIN_PASSWORD near the
# top of this file). None of it is reachable with a normal user's login
# token, and normal user endpoints never accept an admin token.
# =========================================================================
...
def admin_login():
    ...
    # constant-time comparisons so a mistyped admin password can't be
    # narrowed down by response timing the way a naive `==` could be.
    ok = secrets.compare_digest(username, ADMIN_USERNAME) and secrets.compare_digest(password, ADMIN_PASSWORD)
    if not ok:
        return jsonify(error="invalid admin credentials"), 401
    return jsonify(access_token=make_admin_token(), username=ADMIN_USERNAME)


@app.get("/admin/overview")
@admin_auth
def admin_overview(_admin):
    conn = db()
    today = dt.datetime.utcnow().strftime("%Y-%m-%d")
    stats = {
        "total_users": conn.execute("SELECT COUNT(*) c FROM users WHERE is_official=0").fetchone()["c"],
        ...
        "messages_today": conn.execute(
            "SELECT COUNT(*) c FROM messages WHERE sent_at>=?", (today,)).fetchone()["c"],
        "open_reports": conn.execute(
            "SELECT COUNT(*) c FROM reports WHERE status='open'").fetchone()["c"],
        "warnings_issued": conn.execute("SELECT COUNT(*) c FROM warnings").fetchone()["c"],
...
@admin_auth
def admin_activity(_admin):
    conn = db()
    rows = conn.execute(
        "SELECT action,target,detail,created_at FROM admin_actions ORDER BY id DESC LIMIT 100").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
def admin_users(_admin):
    ...
    conn = db()
    rows = conn.execute("""
      SELECT id,username,avatar,country,last_seen,banned,ban_reason,banned_at
      FROM users WHERE is_official=0 AND username LIKE ? ORDER BY username LIMIT 200
    """, (q,)).fetchall()
    result = []
    for row in rows:
        item = dict(row)
        item["banned"] = bool(item["banned"])
        item["online"] = online(row["id"])
        item["report_count"] = conn.execute(
            "SELECT COUNT(*) c FROM reports WHERE reported_id=?", (row["id"],)).fetchone()["c"]
        item["warning_count"] = conn.execute(
            "SELECT COUNT(*) c FROM warnings WHERE user_id=?", (row["id"],)).fetchone()["c"]
        result.append(item)
    conn.close()
    return jsonify(result)
    ...
    if status == "all":
        rows = conn.execute("""
          SELECT r.id,r.reason,r.status,r.created_at,r.resolved_at,r.resolution_note,
                 ...
                 reported.id AS reported_id, reported.banned AS reported_banned
          FROM reports r
        ...
        rows = conn.execute("""
          ...
          JOIN users reporter ON reporter.id=r.reporter_id
          JOIN users reported ON reported.id=r.reported_id
          WHERE r.status=? ORDER BY r.id DESC LIMIT 300
        """, (status,)).fetchall()
    result = []
    for row in rows:
        item = dict(row)
        item["reported_banned"] = bool(item["reported_banned"])
        item["reported_total_reports"] = conn.execute(
            "SELECT COUNT(*) c FROM reports WHERE reported_id=?", (row["reported_id"],)).fetchone()["c"]
        result.append(item)
    conn.close()
    return jsonify(result)
...
@admin_auth
def admin_conversation(_admin):
    """Read-only view of the actual message history between two users, so
    an admin can check a report against what was really said before
    acting on it. Deleted-for-user messages are still shown here (flagged)
    since a deletion by one of the two people involved shouldn't be able
    to hide the evidence a report depends on."""
    user_a, user_b = request.args.get("a", ""), request.args.get("b", "")
    conn = db()
    a = conn.execute("SELECT id,username FROM users WHERE username=?", (user_a,)).fetchone()
    b = conn.execute("SELECT id,username FROM users WHERE username=?", (user_b,)).fetchone()
    if not a or not b:
        conn.close()
        return jsonify(error="one or both users not found"), 404
    rows = conn.execute("""SELECT id,sender_id,receiver_id,type,content,duration,sent_at,
      deleted_for_sender,deleted_for_receiver FROM messages WHERE
      ((sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?))
      ORDER BY id ASC LIMIT 500""", (a["id"], b["id"], b["id"], a["id"])).fetchall()
    conn.close()
    result = [{
        "id": r["id"], "from": a["username"] if r["sender_id"] == a["id"] else b["username"],
        "type": r["type"], "content": r["content"], "duration": r["duration"], "sent_at": r["sent_at"],
        "deleted_for_sender": bool(r["deleted_for_sender"]),
        "deleted_for_receiver": bool(r["deleted_for_receiver"]),
    } for r in rows]
    return jsonify(result)
...
@app.get("/admin/feed")
@admin_auth
def admin_feed(_admin):
    """A recent-messages snapshot for the live feed tab on first load —
    the socket 'admin_new_message' event keeps it updated after that."""
    conn = db()
    rows = conn.execute("""
      SELECT m.id, s.username AS sender, rcv.username AS receiver, m.type, m.content, m.sent_at
      FROM messages m JOIN users s ON s.id=m.sender_id JOIN users rcv ON rcv.id=m.receiver_id
...
def _ban_user(admin_name, name, reason):
    """Shared by the direct ban endpoint and the report-resolution
    shortcut, so both take the exact same effect regardless of which UI
    action triggered it."""
    conn = db()
    target = conn.execute("SELECT id FROM users WHERE username=?", (name,)).fetchone()
    if not target:
        conn.close()
        return None
    conn.execute("""UPDATE users SET banned=1, ban_reason=?, banned_at=CURRENT_TIMESTAMP
                    WHERE id=?""", (reason, target["id"]))
    conn.commit()
    conn.close()
    # Kick every live connection that account has right now.
    for sid_ in list(online_users.get(target["id"], set())):
        socketio.emit("account_banned", {"reason": reason}, to=sid_)
        socketio.disconnect(sid_)
    log_admin_action(admin_name, "ban", name, reason)
    socketio.emit("admin_user_banned", {"username": name, "reason": reason}, room="admin_live")
    return True


@app.post("/admin/users/<name>/ban")
@admin_auth
def admin_ban_user(admin_name, name):
    reason = str((request.get_json(silent=True) or {}).get("reason") or "").strip()[:500]
    if _ban_user(admin_name, name, reason) is None:
        return jsonify(error="no such user"), 404
    return jsonify(banned=True, username=name)


@app.post("/admin/users/<name>/unban")
@admin_auth
def admin_unban_user(admin_name, name):
    conn = db()
    cur = conn.execute("""UPDATE users SET banned=0, ban_reason=NULL, banned_at=NULL
                          WHERE username=?""", (name,))
    conn.commit()
    conn.close()
    if not cur.rowcount:
        return jsonify(error="no such user"), 404
    log_admin_action(admin_name, "unban", name)
    return jsonify(banned=False, username=name)


def _warn_user(admin_name, name, message):
    """Shared by the direct warn endpoint and the report-resolution
    shortcut. A warning is only ever readable by the account owner —
    GET /account/warnings scopes strictly to the caller's own user_id."""
    if not message:
        return None
    conn = db()
    target = conn.execute("SELECT id FROM users WHERE username=?", (name,)).fetchone()
    if not target:
        conn.close()
        return None
    conn.execute("INSERT INTO warnings(user_id,message,issued_by) VALUES(?,?,?)",
                (target["id"], message, admin_name))
    conn.commit()
    conn.close()
    # In-app if they're online right now, plus a push so it still reaches
    # them if the app is closed — "admin" isn't a real notification-prefs
    # category, so notification_prefs().get("admin", True) always falls
    # through to True regardless of what the user has muted.
    socketio.emit("account_warning", {"message": message}, room=f"user_{target['id']}")
    send_push(target["id"], "admin", "Account notice", message, url="/settings/account")
    log_admin_action(admin_name, "warn", name, message)
    return True


@app.post("/admin/users/<name>/warn")
@admin_auth
def admin_warn_user(admin_name, name):
    message = str((request.get_json(silent=True) or {}).get("message") or "").strip()[:1000]
    if not message:
        return jsonify(error="a warning message is required"), 400
    if _warn_user(admin_name, name, message) is None:
        return jsonify(error="no such user"), 404
    return jsonify(warned=True, username=name)


@app.post("/admin/reports/<int:report_id>/resolve")
@admin_auth
def admin_resolve_report(admin_name, report_id):
    data = request.get_json(silent=True) or {}
    action = data.get("action", "dismiss")  # dismiss | warn | ban
    note = str(data.get("note") or "").strip()[:500]
    if action not in ("dismiss", "warn", "ban"):
        return jsonify(error="action must be dismiss, warn, or ban"), 400
    conn = db()
    report = conn.execute("""SELECT r.*, reported.username AS reported_username
      FROM reports r JOIN users reported ON reported.id=r.reported_id WHERE r.id=?""",
                          (report_id,)).fetchone()
    if not report:
        conn.close()
        return jsonify(error="report not found"), 404
    if action == "warn" and not note:
        conn.close()
        return jsonify(error="a warning message (note) is required for this action"), 400
    conn.execute("""UPDATE reports SET status=?, resolved_at=CURRENT_TIMESTAMP, resolution_note=?
                    WHERE id=?""", ("dismissed" if action == "dismiss" else "actioned", note, report_id))
    conn.commit()
    conn.close()
    target_name = report["reported_username"]
    if action == "warn":
        _warn_user(admin_name, target_name, note)
    elif action == "ban":
        _ban_user(admin_name, target_name, note or "Actioned from a user report")
    log_admin_action(admin_name, f"resolve-report:{action}", target_name, note)
    return jsonify(resolved=True, action=action)


@app.post("/admin/notify")
@admin_auth
def admin_notify(admin_name):
    """Send an in-chat message from the official HwFar account — to one
    or more specific usernames, or to every user ("targets": "all")."""
    data = request.get_json(silent=True) or {}
    ...
    conn = db()
    official_id = official_user_id(conn)
    if not official_id:
        conn.close()
        return jsonify(error="official account is not set up"), 500
    if targets == "all":
        rows = conn.execute("SELECT id,username FROM users WHERE is_official=0 AND banned=0").fetchall()
    else:
        names = [str(t).strip() for t in (targets or []) if str(t).strip()]
        marks = ",".join("?" for _ in names) or "''"
        rows = conn.execute(f"SELECT id,username FROM users WHERE username IN ({marks})", names).fetchall() if names else []
    sent_to = []
    for target in rows:
        conn.execute("INSERT INTO contacts(owner_id,contact_id) VALUES(?,?) "
                    "ON CONFLICT (owner_id,contact_id) DO NOTHING", (target["id"], official_id))
        conn.execute("INSERT INTO contacts(owner_id,contact_id) VALUES(?,?) "
                    "ON CONFLICT (owner_id,contact_id) DO NOTHING", (official_id, target["id"]))
        delivered = 1 if online(target["id"]) else 0
        cur = conn.execute("""INSERT INTO messages(sender_id,receiver_id,type,content,delivered)
                             VALUES(?,?,?,?,?) RETURNING id""",
                           (official_id, target["id"], "text", message, delivered))
        new_id = cur.fetchone()["id"]
        conn.commit()
        row = conn.execute("SELECT * FROM messages WHERE id=?", (new_id,)).fetchone()
        socketio.emit("new_message", {
            "id": row["id"], "from": OFFICIAL_USERNAME, "type": "text", "content": message,
            "duration": None, "sent_at": row["sent_at"], "delivered": bool(delivered),
            "read": False, "expires_at": None, "edited": False, "deleted": False,
        }, room=f"user_{target['id']}")
        send_push(target["id"], "admin", OFFICIAL_USERNAME, message[:150],
                  url=f"/chat/{urllib.parse.quote(OFFICIAL_USERNAME)}", tag="official")
        sent_to.append(target["username"])
    conn.close()
    log_admin_action(admin_name, "notify", "all" if targets == "all" else ",".join(sent_to), message)
    return jsonify(sent=True, recipients=sent_to, count=len(sent_to))


@app.post("/admin/push")
@admin_auth
def admin_push(admin_name):
    """Device push only (no chat message) — to one/several usernames or
    every device ("targets": "all"). Reuses send_push()'s FCM delivery,
    with the same "admin" pseudo-category trick as warnings so this is
    never silenced by a user's per-category notification prefs."""
    data = request.get_json(silent=True) or {}
    title = str(data.get("title") or OFFICIAL_USERNAME).strip()[:80]
    body = str(data.get("body") or "").strip()[:200]
    targets = data.get("targets", "all")
    if not body:
        return jsonify(error="a notification body is required"), 400
    conn = db()
    if targets == "all":
        rows = conn.execute("SELECT id,username FROM users WHERE is_official=0 AND banned=0").fetchall()
    else:
    ...
    conn.close()
    for target in rows:
        send_push(target["id"], "admin", title, body, url="/chat", tag="admin-push")
    log_admin_action(admin_name, "push", "all" if targets == "all" else ",".join(r["username"] for r in rows),
                     f"{title}: {body}")
    return jsonify(sent=True, count=len(rows))


@socketio.on("admin_subscribe")
def admin_subscribe(data):
    """The admin panel's live feed joins this room after authenticating
    with its own admin token over the socket (never a user token)."""
    admin_name = admin_from_token((data or {}).get("token", ""))
    if not admin_name:
        return
...
def create_status(user_id):
    # Two ways in: multipart/form-data (real file bytes — WhatsApp-style,
    # what the browser sends now for image/video/audio statuses) or plain
    # JSON (still accepted for text statuses, and for any older client).
    # The old path shipped every photo/video/audio status as base64 text
    # inside one JSON body — a 10MB video became a 13MB+ JSON string the
    # browser had to build on its main thread before sending, which is slow
    # enough on a phone to look like the upload "isn't working" even when
    # nothing actually errors. Multipart sends the real bytes directly.
    is_multipart = bool(request.files) or (
        request.content_type and "multipart/form-data" in request.content_type
    )
    if is_multipart:
        kind = request.form.get("type", "text")
        caption = (request.form.get("caption") or "").strip()
        audience = request.form.get("audience", "everyone")
        allowed_raw = request.form.getlist("allowed_users")
        hidden_raw = request.form.getlist("hidden_users")
        ...
        if kind == "text":
            ...
            if not upload or not upload.filename:
                return jsonify(error="choose or record media first"), 400
            raw = upload.read()
            if not raw:
                return jsonify(error="that file looked empty — try again"), 400
            if len(raw) > 12_000_000:
                return jsonify(error="media must be smaller than 12 MB"), 413
            default_mime = {"image": "image/jpeg", "video": "video/mp4", "audio": "audio/webm"}[kind]
            mime = upload.mimetype or default_mime
        ...
        allowed = audience_names(allowed_raw)
        hidden = audience_names(hidden_raw)
    else:
        data = request.get_json(silent=True) or {}
        ...
        if len(caption) > 280:
            return jsonify(error="caption must be 280 characters or less"), 400
        content = content.strip() if kind == "text" else content
    ...
    cur = conn.execute("""INSERT INTO statuses
      ...
      VALUES(?,?,?,?,?,?,?,?) RETURNING id""", (
        user_id, kind, content, caption, audience,
...
def send(user_id):
    ...
    socketio.emit("admin_new_message", {
        "id": row["id"], "sender": sender["username"], "receiver": to,
    ...
    conn2 = db()
    mute_row = conn2.execute("SELECT muted FROM contacts WHERE owner_id=? AND contact_id=?",
                             (receiver["id"], user_id)).fetchone()
    conn2.close()
    if not (mute_row and mute_row["muted"]):
        # Muting a contact (like WhatsApp) only silences push alerts — the
        # message still sends, delivers, and ticks normally either way.
        ...
        send_push(
            ...
            url=f"/chat/{urllib.parse.quote(sender['username'])}",
...
@app.get("/messages/<other>/media")
@auth
def shared_media(user_id, other):
    conn = db()
    purge(conn)
    ...
    if not target:
        conn.close()
        return jsonify(error="no such user"), 404
    rows = conn.execute("""SELECT id,type,content,sent_at FROM messages WHERE
      type IN ('image','video') AND (
        (sender_id=? AND receiver_id=? AND deleted_for_sender=0) OR
        (sender_id=? AND receiver_id=? AND deleted_for_receiver=0)
      ) ORDER BY id DESC""", (user_id, target["id"], target["id"], user_id)).fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
def _finalize_offline(user_id):
    """Runs once PRESENCE_GRACE_SECONDS after a user's last socket dropped.
    If they reconnected in the meantime (any HwFar page — chat, settings,
    stories all keep a socket alive) this is a no-op; online_users will
    already have their new sid."""
    pending_offline.pop(user_id, None)
...
@app.get("/admin")
def admin_page():
    """Serves the admin panel shell itself. The page is a login gate — no
    server-side session check here — every API call it makes carries its
    own admin bearer token, so nothing sensitive is reachable just by
    loading this URL."""
...
def socket_connect(auth_data):
    # The admin panel connects with an admin_token instead of a regular
    # user token — let it through here (no user room to join) and rely on
    # the admin_subscribe event below to put it in the admin_live room,
    # since admin_from_token/user_from_token are deliberately two
    # non-overlapping token types.
    if admin_from_token((auth_data or {}).get("admin_token", "")):
        return
    ...
    timer = pending_offline.pop(user_id, None)
    if timer:
        # They reconnected inside the grace window (page navigation, a
        # locked screen waking up, a brief drop) — cancel the scheduled
        # "offline" broadcast so contacts never saw them leave.
        timer.cancel()
    conn = db()
...
def socket_disconnect():
    ...
    if not online_users.get(user_id):
        ...
        old_timer = pending_offline.pop(user_id, None)
        if old_timer:
            old_timer.cancel()
        # Don't mark them offline yet — schedule it for after the grace
        # window and only go through with it if they still haven't
        # reconnected by then (see _finalize_offline).
        pending_offline[user_id] = eventlet.spawn_after(PRESENCE_GRACE_SECONDS, _finalize_offline, user_id)
