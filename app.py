...
try:
    ...
    if _fb_key_json and not firebase_admin._apps:
        firebase_admin.initialize_app(fb_credentials.Certificate(json.loads(_fb_key_json)))
    if not _fb_key_json:
        fb_messaging = None
    ...
    for sub in subs:
        ...
        try:
            fb_messaging.send(fb_messaging.Message(
                ...
                data={"url": url, "tag": tag or category},
                token=sub["fcm_token"],
...
if __name__ == "__main__":
    ...
    if not os.environ.get("FIREBASE_VAPID_KEY"):
        print("[push] FIREBASE_VAPID_KEY not set — the browser can't "
              "register for push even with FIREBASE_SERVICE_ACCOUNT_JSON "
              "configured. Get it from Firebase console → Project Settings "
