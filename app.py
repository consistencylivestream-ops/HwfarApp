...
def pg_connect():
    ...
    conn.execute = _pg_execute.__get__(conn)
    return conn
...
def auth_strict(handler):
    """Like @auth, but never falls back to the first registered account.

    Reserved for endpoints that change credentials (password, recovery
    email, avatar): those must only ever act on the real bearer token's
    owner, never on a guessed "demo user" when no token is presented.
    """
    @wraps(handler)
    def wrapped(*args, **kwargs):
        header = request.headers.get("Authorization", "")
...
def send_email(to_addr, subject, body):
    ...
    if not host:
        ...
        print(f"[DEV EMAIL - NOT ACTUALLY SENT, SMTP_HOST is unset] "
...
def account_me(user_id):
    ...
    row = conn.execute("""SELECT id,username,language,avatar,country,age,gender,
      ...
      show_status_reactions,welcome_animation,email,email_verified,google_sub
      FROM users WHERE id=?""",
                       (user_id,)).fetchone()
    conn.close()
    if not row:
...
@app.get("/new-chat")
@auth
def new_chat_page(user_id): return render_template("new_chat.html", prefill_to=request.args.get("to", ""))
